<template>
  <div class="distinct-component-ct">
    <el-select :style="{
      ...
      }" :size="size" v-if="layoutLevels.includes(1)" placeholder="请选择省份" filterable v-model="provinceSelect"
      @change="provinceChange">
      <el-option v-for="(item, index) in provinceArr" :key="index" :label="item" :value="item">
      </el-option>
    </el-select>
    <el-select :style="{
          width: selectWidth + unit,
          marginRight: layoutLevels.includes(3) ? (spaceWidth + unit) : 0
        }" :size="size" v-if="layoutLevels.includes(2)" placeholder="请选择市" filterable v-model="citySelect"
      @change="cityChange">
      <el-option v-for="(item, index) in computingCitys" :key="index" :lable="index" :value="index">
      </el-option>
    </el-select>
    <el-select :style="{width: selectWidth + unit}" :size="size" v-if="layoutLevels.includes(3)" placeholder="请选择区"
      filterable v-model="areaSelect" @change="areaChange">
      <el-option v-for="(item, index) in computingAreas" :label="item" :key="index" :value="item">
      </el-option>
    </el-select>
  </div>
</template>
